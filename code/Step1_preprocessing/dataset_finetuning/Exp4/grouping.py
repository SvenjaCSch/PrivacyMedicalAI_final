...
class Grouping:
    def __init__(self):
        ...
        self.children = Personal_children()
    ...
    def generate_weighted_random_dataset(self, combined_groups:dict, combined_indices:dict, gender_weights:dict, person_type:str)->pd.DataFrame:
        ...
        for _ in range(12000):
            ...
            if not eligible_keys:
                print(f"Warning: All eligible groups exhausted for {person_type} after {len(final_df)} entries.")
                break

            selected_key = self.select_group_key(eligible_keys, gender_weights)

            if not selected_key:
                print(f"No valid key could be selected for {person_type} based on weights. Skipping this iteration.")
                continue
            ...
            if start_idx >= len(group_df):
                print(f"Warning: Group {selected_key} exhausted. Skipping it.")
                del combined_groups[selected_key]
                del combined_indices[selected_key]
                continue

            person_row = group_df.iloc[start_idx]
            combined_indices[selected_key] += 1

            if person_type == "adults":
                new_entry_df = self.adults.make_dataframe_first(person_row, selected_key[1]) # Pass Series and age_group
            else:
                new_entry_df = self.children.make_dataframe_first(person_row, selected_key[1]) # Pass Series and age_group

            final_df = pd.concat([final_df, new_entry_df], ignore_index=True)
            person_id += 1
    ...
    def select_group_key(eligible_keys:list, gender_weights:dict)->str|None:
        ...
        eligible_genders = [g for g in eligible_genders if gender_weights.get(g, 0) > 0]
    ...
    def build_group_entry(self, person_rows: pd.Series, selected_key: int,person_type:str)->pd.DataFrame:
        ...
        if person_type == "adults":
            first_row_df = self.adults.make_dataframe_first(person_rows, selected_age_group)
        else:
            first_row_df = self.children.make_dataframe_first(person_rows, selected_age_group)
    ...
    def manage_dataset(self, med_data:pd.DataFrame, person_type:str)->pd.DataFrame:
        ...
        combined_groups = self.create_combined_groups(med_data)
