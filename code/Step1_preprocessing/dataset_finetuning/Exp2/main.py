...
from personal_data_adults import PersonalAdults
from personal_data_children import PersonalChildren
from combine_datasets import Combine
from code.Code_OLD.old_datasets.disease_symptom_data import Disease
...
class Main:
    ...
    def manage_data(self, base: str, adults_url:str, child_plus_symptoms_url:str, adult_symptoms_url:str, disease_url:str)-> None:
        ...
        combined_phi.to_csv(f"{base}\dataset_multiple_entries_per_person.csv")
        train_shuffled.to_csv(f"{base}\dataset_multiple_entries_per_person_train_shuffled.csv")
        test_shuffled.to_csv(f"{base}\dataset_multiple_entries_per_person_test_shuffled.csv")

if __name__ == "__main__":
    main = Main()
    base = rf"{main.BASE}data\Step1_preprocessing\dataset_finetuning\Exp2"
